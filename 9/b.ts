...
const printGrid = (grid: string[][]): void =>
  grid.forEach((row) => console.log(row.join(" "), row.length));
...
const numberOfVisitedPositions = visitedPositions.reduce(
  (accumulator, row) =>
    accumulator +
    row.reduce((rowAccumulator, position) =>
      rowAccumulator + ((position === "#" || position === "s") ? 1 : 0), 0),
  0,
);
