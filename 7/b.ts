...
const isEmptyDirectory = (item: Directory | File): boolean =>
  "content" in item && item.content.size === 0;
const isContentfulDirectory = (item: Directory | File): boolean =>
  "content" in item && item.content.size > 0;
...
const printDirectory = (directory: Directory, level = 0): void => {
  ...
  for (const item of directory.content.values()) {
    ...
    if (isContentfulDirectory(item)) {
      ...
      console.log(
        `${" ".repeat(level * 2)}- ${item.name} (file, size=${item.size})`,
      );
    }
  }
};

const calculateSizeOfDirectory = (
  directory: Directory,
): { size: number; directories: Directory[] } => {
  let size = 0;
  ...
  for (const item of directory.content.values()) {
    ...
    if (isContentfulDirectory(item)) {
      const { size: directorySize, directories: intermediateDirectories } =
        calculateSizeOfDirectory(item as Directory);
      size += directorySize;
