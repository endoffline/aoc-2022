...
const initializeBlankGrid = (lengthX: number, lengthY: number): number[][] => Array.from({length: lengthX}, () => Array.from({length: lengthY}, () => 0));

const measureScenicScore = (grid: number[][], posY: number, posX: number): number => {
...
const scenicScores: number[][] = initializeBlankGrid(grid.length, grid[0].length);
